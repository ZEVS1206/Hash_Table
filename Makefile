CC=g++
NASM=nasm
NASMFLAGS=-f elf64 -g -F dwarf  # Добавлены отладочные символы для ассемблера

# Основные флаги компиляции (существующие флаги сохранены)
...
# Новые флаги для ассемблерных вставок
CFLAGS += -march=native -fno-pie -mno-red-zone  # Добавлены без изменения исходных

# Флаги для профилирования (без sanitizers)
PROFILE_CFLAGS = -ggdb3 -std=c++17 -O0 -Wall -Wextra -fno-omit-frame-pointer
PROFILE_CFLAGS += -march=native -fno-pie  # Добавлены для профилируемой версии
...
# Новые цели для ассемблерной отладки
...
$(BUILD_DIR)/$(EXEC_NAME): $(OBJECTS) $(ASM_OBJECTS) | $(BUILD_DIR)
	$(CC) $(CFLAGS) $^ -o $@ -no-pie  # Добавлен -no-pie для линковки
